Gem::Specification.new do |s|
  s.name = 'graphql_includable'
  s.version = '0.2.12'
  s.licenses = ['MIT']
  s.summary = 'An ActiveSupport::Concern for GraphQL Ruby to eager-load query data'
  s.authors = ['Dan Rouse', 'Josh Vickery']
  s.email = ['dan.rouse@squarefoot.com', 'jvickery@squarefoot.com']
  s.files = ['lib/graphql_includable.rb', 'lib/graphql_includable/concern.rb', 'lib/graphql_includable/edge.rb', 'lib/graphql_includable/resolver.rb']
  s.homepage = 'https://github.com/thesquarefoot/graphql_includable'
end
