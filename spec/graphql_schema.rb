...
ClientType = GraphQL::ObjectType.define do
  ...
  connection :over_fetched, TaskType.define_connection_with_fetched_edge(edge_type: ClientTaskEdgeType) { name 'OverFetchedConnection' } do
  ...
  connection :nested_query, TaskType.define_connection_with_fetched_edge(edge_type: ClientTaskEdgeType) { name 'NestedQueryConnection' } do
    connection_properties(edge_to_node: :task)
